function plotFigure3(Parray, R, S, Narray)
%PLOTFIGURE3 This method plots figure 3 from the paper
...
for s = 1:S 
    ...
    if s==1
        ...
        orbit_uniform_error   = max(obj.orbit_uniform_error(:,:,1:end-2),[],3);
        snippet_uniform_error = max(obj.snippet_uniform_error(:,:,1:end-2),[],3);
    end

    orbit_lsw_error(:,:,:,s)    = max(obj.orbit_lsw_tikhonov_error(:,:,:,1:end-2),[],4); % *_error(:,:,:,end) is the lyapunov exp err. Lets throw away this obs to compute E_max over \mathcal{B}
    orbit_markov_error(:,:,:,s) = max(obj.orbit_markov_error(:,:,:,1:end-2),[],4);

    snippet_lsw_error(:,:,:,s)    = max(obj.snippet_lsw_tikhonov_error(:,:,:,1:end-2),[],4);
    snippet_markov_error(:,:,:,s) = max(obj.snippet_markov_error(:,:,:,1:end-2),[],4);
...
exportgraphics(gcf,'media/fig3a.pdf','ContentType','vector');
...
exportgraphics(gcf,'media/fig3b.pdf','ContentType','vector');
...
exportgraphics(gcf,'media/fig3c.pdf','ContentType','vector');
...
exportgraphics(gcf,'media/fig3d.pdf','ContentType','vector');
