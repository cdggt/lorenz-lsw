...
%UNORDERED_POT_WEIGHTS this method computes pot
% weights of the orbits in P_r
%
% Inputs:
%
%   orbitIndices  : an array of orbits' indices to be used in the weighted
%                 POT expansion

% Please see pot.pdf for a more detailed description, the comments in this 
% script will reference that document 
    ...
    % run a newton solver to find s such that F(s,0) = 0, see ( Eq 4 of pot.pdf )
    s = 0; % initial guess
    maxIter = 1e5;
    for i = 1:maxIter
    ...
    % compute weights using Eq 2 of pot.pdf
...
function [Q,dQ_ds,d2Q_dbeta_dmu] = compute_determinant_derivatives(orbitIndices,s,maxtopologicalperiod)
    ...
    for p = 1:P
        ...
        % for each repeat, r, of orbit p, such that r*n_p = i (see Eq. 3 of pot.pdf)
        for r = 1:floor(maxtopologicalperiod/orbit.topologicalperiod)
            
            % compute stability of each orbit det(1-M_p^r). see just below Eq. 3 of pot.pdf
            ...
            % add term to trace coefficient sum (Eq 3 of pot.pdf)
            C(j) = C(j) + weight;

            % add term to trace coefficient derivative sum (Eq 6 of pot.pdf)
            dC_ds(j) = dC_ds(j) + weight * (-r * period);

            % add term to trace coefficient derivative sum (Eq 8 of pot.pdf)
